...
internal class RedisRepositoryTest {
    ...
    @Test
    fun `Given that 1 element is inserted in Redis, When getMap() is called, it Should return something`() {
    ...
    @Test
    fun `Given that a map with 2 elements is inserted in Redis, When clearAll() is called, redis Should be empty`() {
        val jsonMap = mutableMapOf<String, Currency>()
        jsonMap["DOGE"] = Currency(BigDecimal(0.777))
        jsonMap["CAD"] = Currency(BigDecimal(3.23))
        redisRepository.setMap(jsonMap)
